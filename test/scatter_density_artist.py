...
class ScatterDensityArtist(AxesImage):
    """
    Matplotlib artist to make a density plot of (x, y, z) scatter data.
    ...
    def __init__(self, ax, x, y, z, dpi=72, downres_factor=4, color=None, **kwargs):
    ...
    def get_window_extent(self, renderer=None):
        x0, x1, y0, y1, z0, z1 = self.get_extent()
        bbox = Bbox.from_extents([x0, y0, x1, y1, z0, z1])
    ...
    def make_image(self, *args, **kwargs):
        ...
        if self._downres:
            ...
            nz_sub = nz // self._downres_factor
            array = histogram3d(self._z_sub, self._y_sub, self._x_sub,
                                bins=(nz_sub, ny_sub, nx_sub),
                                range=((zmin,zmax), (ymin, ymax), (xmin, xmax)))
        else:
            array = histogram3d(self._z, self._y, self._x,
                                bins=(nz, ny, nx),
                                range=((zmin,zmax), (ymin, ymax), (xmin, xmax)))
