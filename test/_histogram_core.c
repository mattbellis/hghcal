...
static PyMethodDef module_methods[] = {
        ...
        {"_histogram3d", _histogram3d, METH_VARARGS, _histogram3d_docstring},
        {NULL, NULL, 0, NULL}
};
...
static PyObject *_histogram3d(PyObject *self, PyObject *args)
{
        ...
        double xmin, xmax, tx, fnx, normx, ymin, ymax, ty, fny, normy, zmin, zmax, tz, fnz, normz;
        PyObject *x_obj, *y_obj, *z_obj, *x_array, *y_array, *z_array, *count_array;
        npy_intp dims[2];
        double *x, *y, *z, *count;

        /* Parse the input tuple */
        if (!PyArg_ParseTuple(args, "OOiddidd", &x_obj, &y_obj, &z_obj, &nx, &xmin, &xmax, &ny, &ymin, &ymax, &nz, &zmin, &zmax)) {
                PyErr_SetString(PyExc_TypeError, "Error parsing input");
                return NULL;
        }
        ...
        x_array = PyArray_FROM_OTF(x_obj, NPY_DOUBLE, NPY_IN_ARRAY);
        y_array = PyArray_FROM_OTF(y_obj, NPY_DOUBLE, NPY_IN_ARRAY);
        z_array = PyArray_FROM_OTF(z_obj, NPY_DOUBLE, NPY_IN_ARRAY);
        ...
        if (n != (long)PyArray_DIM(z_array, 0)) {
                 PyErr_SetString(PyExc_RuntimeError, "Dimension mismatch between x and z");
                 Py_DECREF(x_array);
                 Py_DECREF(z_array);
                 return NULL;
        }
        if (n2 != (long)PyArray_DIM(z_array, 0)) {
                 PyErr_SetString(PyExc_RuntimeError, "Dimension mismatch between y and z");
                 Py_DECREF(y_array);
                 Py_DECREF(z_array);
                 return NULL;
        }
        ...
        count_array = PyArray_SimpleNew(3, dims, NPY_DOUBLE);
        if (count_array == NULL) {
                 PyErr_SetString(PyExc_TypeError, "Couldn't build output array");
                 Py_DECREF(x_array);
                 Py_XDECREF(count_array);
        ...
        for(i = 0; i < n; i++) {
                ...
                if (tx >= xmin && tx < xmax && ty >= ymin && ty < ymax && tz >= zmin && tz < xmax) {
                        ix = (tx - xmin) * normx * fnx;
                        iy = (ty - ymin) * normy * fny;
                        iz = (tz - zmin) * normz * fnz;
                        count[iy + ny * ix] += 1.;
                }
